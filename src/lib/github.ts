 ...
 * Returns entries sorted by path descending (newest dates first).
 */
...
/**
 * Get the decoded text content of a file from the repo.
 */
export async function getFileContent(
  ...
  if (!resp.ok) {
    const errText = await resp.text();
    throw new Error(`GitHub API error reading file (${resp.status}): ${errText}`);
  }
